...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        """
            ...
            self.vars.create_var('v_previous', 0.0) #decalaration

            Example: Setting 'v_previous' to be 1.0
            self.vars.v_previous = 1.0 #access or setup
        ...
        if self._start_control_loop:
            ...
            error_v = v_desired - v     #error (desired velocity- velocity)

            integral = self.vars.integral_error_previous + error_v * dt  # integration

            derivative = (error_v - self.vars.error_previous) / dt  #derivate

            acceleration = kp*error_v + ki * integral + kd * derivative           #ac=kp(vd-v)+ki(vd-v)*dt+kd d(vd-v)/dt

            if acceleration > 0:
                throttle_output = (np.tanh(acceleration) + 1) / 2
            ...
            #lets try Stanley Controller  delta=yaw(t)+np.arctan2(k*e(t)/v)

            k_e = 0.3
            k_v=10 #softening condition
            ...
            yaw_diff_crosstrack = np.arctan(k_e * crosstrack_error/(v))#yaw_diff_crosstrack=np.arctan(k_e*crosstrack_error/(k_v+v));k_v os softening constant
        ...
        self.vars.integral_error_previous=integral
        self.vars.error_previous=error_v
